/* Autogenerated file. Do not edit manually. */
/* tslint:disable */
/* eslint-disable */
import type {
  BaseContract,
  BigNumber,
  BytesLike,
  CallOverrides,
  PopulatedTransaction,
  Signer,
  utils,
} from "ethers";
import type { FunctionFragment, Result } from "@ethersproject/abi";
import type { Listener, Provider } from "@ethersproject/providers";
import type {
  TypedEventFilter,
  TypedEvent,
  TypedListener,
  OnEvent,
  PromiseOrValue,
} from "./common";

export interface PopcornInterface extends utils.Interface {
  functions: {
    "var1()": FunctionFragment;
  };

  getFunction(nameOrSignatureOrTopic: "var1"): FunctionFragment;

  encodeFunctionData(functionFragment: "var1", values?: undefined): string;

  decodeFunctionResult(functionFragment: "var1", data: BytesLike): Result;

  events: {};
}

export interface Popcorn extends BaseContract {
  connect(signerOrProvider: Signer | Provider | string): this;
  attach(addressOrName: string): this;
  deployed(): Promise<this>;

  interface: PopcornInterface;

  queryFilter<TEvent extends TypedEvent>(
    event: TypedEventFilter<TEvent>,
    fromBlockOrBlockhash?: string | number | undefined,
    toBlock?: string | number | undefined
  ): Promise<Array<TEvent>>;

  listeners<TEvent extends TypedEvent>(
    eventFilter?: TypedEventFilter<TEvent>
  ): Array<TypedListener<TEvent>>;
  listeners(eventName?: string): Array<Listener>;
  removeAllListeners<TEvent extends TypedEvent>(
    eventFilter: TypedEventFilter<TEvent>
  ): this;
  removeAllListeners(eventName?: string): this;
  off: OnEvent<this>;
  on: OnEvent<this>;
  once: OnEvent<this>;
  removeListener: OnEvent<this>;

  functions: {
    var1(overrides?: CallOverrides): Promise<[BigNumber]>;
  };

  var1(overrides?: CallOverrides): Promise<BigNumber>;

  callStatic: {
    var1(overrides?: CallOverrides): Promise<BigNumber>;
  };

  filters: {};

  estimateGas: {
    var1(overrides?: CallOverrides): Promise<BigNumber>;
  };

  populateTransaction: {
    var1(overrides?: CallOverrides): Promise<PopulatedTransaction>;
  };
}
